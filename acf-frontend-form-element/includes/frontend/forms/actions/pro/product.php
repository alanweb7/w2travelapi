	...
	public function get_fields_display( $form_field, $local_field, $wg_id = '', $sub_fields = false, $saving = false ){
		...
		foreach( array( 'simple', 'external', 'grouped', 'variable' ) as $_type ){
			$product_types[$_type] = array(
				'field' => 'acfef_' . $wg_id . '_product_type',
				'operator' => '==',
				'value' => $_type,
			);	
		}
		foreach( array( 'simple', 'external', 'grouped', 'variable' ) as $_type ){
			$product_types['not_' .$_type] = array(
				'field' => 'acfef_' . $wg_id . '_product_type',
				'operator' => '!=',
				'value' => $_type,
			);	
		...
		switch( $form_field['field_type'] ){
			...
			break;
			case 'attributes':
				$form_field = acfef_parse_args( $form_field, array(
					'button_text' => '',
					'save_button_text' => '',
					'no_value_msg' => '',
				) );
				...
				if( is_array( $sub_fields ) ){
					...
					foreach( $sub_fields as $i => $sub_field ){
						$sub_fields[$i] = acfef_parse_args( $sub_fields[$i], $sub_settings );		
					}			
				}
	...
	public function register_settings_section( $widget ) {
        ...
        $widget->add_control( 'delete_message_product', [
			...
			'condition' => [
				'main_action' => 'edit_product',
				'show_product_delete_button' => 'true',
				'show_delete_message_product' => 'true'
			],
		] );
		$widget->add_control(
			...
			[
				...
				'default' => 'true',
				'return_value' => 'true',
				'description' => __( 'Whether or not to completely delete the product right away.' ),
				'condition' => [
					'main_action' => 'edit_product',
					'show_product_delete_button' => 'true',
				],
