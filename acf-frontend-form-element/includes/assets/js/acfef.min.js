...
c("body").on("click",".modal .close",function(a){a=c(this).parents(".modal");1==a.data("clear")&&(a.remove(),modalLevel--,narrowfy-=20)});acf.add_filter("validation_complete",function(a,b){a.errors&&(c(".acf-loading").remove(),b.find("input[type=submit]").removeClass("disabled"));return a});c("body").on("click","form.acfef-form.-submit input[type=submit]",function(a){$form=c(this).parents("form");$form.find("input[name=_acf_status]").val(c(this).data("state"));c(this).hasClass("disabled")||c(this).after('<span class="acf-loading"></span>')});
c("body").on("submit","form.-delete",function(a){a.preventDefault();$form=c(this);$form.find("button[type=submit]").addClass("disabled").after('<span class="acf-loading"></span>');a=new FormData($form[0]);a.append("action","acfef/delete_object");c.ajax({url:acf.get("ajaxurl"),type:"post",data:a,cache:!1,processData:!1,contentType:!1,success:function(b){b.success&&b.data.redirect&&(window.location=b.data.redirect)}})});c("body").on("submit","form.-submit",function(a){a.preventDefault();$form=c(this);
...
b.parents(".modal").remove()}else b.find(".acf-loading").remove(),b.find(".acfef-submit-button").attr("disabled",!1).removeClass("acf-hidden"),f.data.clear_form&&(g=b.data("widget"),b.siblings(".acfef-form-posts").remove(),b.replaceWith(f.data.clear_form),b=c(".elementor-element-"+g).find("form"),acf.do_action("append",c(".elementor-element-"+g)),f.data.saved_message&&(b.find(".save-progress-button").after('<p class="draft-saved">'+f.data.saved_message+"</p>"),setTimeout(function(){c("body").find(".draft-saved").remove()},
